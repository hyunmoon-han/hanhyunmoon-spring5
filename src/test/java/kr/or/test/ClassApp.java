 ...
 * 2.오브젝트(객체)생성에 필요한 new키워드(예약어) 생성자 메서드 사용연습,
 * 3.추상클래스(Abstract) 대한 연습
 *  * @author  현문
 *
 */
//오브젝트생성에 필요한 new 키워드 생성자 메서드사용한 실습 클래스(아래)
class Circle {
	private int r;//원의 반지름으로 사용할 변수
	// 생성자 메서드(행위) /아래
	//public Circle() {} 만들지 않아도 자동으로 만들어 집니다. 
	public Circle(int radius) {
		r = radius;//Get|Set중에 Set저장 멤버변수 처리
	}
	//원의 넓이를 구하는 메서드 반환(리턴)값의 크기가 더블형 (아래)
	public double getCircle() {
		double result = r*r*3.14;//원의 넓이를 구한느 계산식
		return result;
	}
}
//클래스 상속에 대한 연습(아래)
class Employee {//고용(사원,취업) 부모 클래스
	int mSalary;//m은 멤버변수  사원에대한월급 변수
...
class Salesman extends Employee{//extends(확장)->상속
	public Salesman( ) {//클래스 명과 똑같은 이름의 메서드가 생성자 함수입니다.
		...
		mDept = "팬매부서";//부모클래스의 변수를 상속받아서 사용가능.
	}
	public void doJob() {
...
abstract class GraphicObject{
	int x,y;
	abstract void draw();//구현 내용이 없고 ,메서드 명만 존재.추상메서드입니다.
	//추상메서드를 만드는 이유는 메서드가 수십개 수백개일때 개발자가 구현할때 어려움이 존재
	//위 문제를 해소한 기능의 클래스  수백개의 메서드를 이름만 모아서 , 유지 보수가 편리하게 처리한 방식입니다.
}
//추상클래스를 사용하는 방법(아래)
//스프링에서는 추상클래스보다는 인터페이스를 더 많이 사용합니다. 사용의도는 비슷합니다.
class Traiangle extends GraphicObject{
	//삼각형 그리기
	@Override//부모클래스의 메서드를 재정의해서 사용  = 오버라이드/오버로드 차이점확인(step1확인)
	void draw() {
		//삼각형을 만듬
		System.out.println("  *");
		System.out.println(" * *");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		//개잘자가 입력한 반지름의 원의 넓이를 구한느 오브젝트를 생성(아래)
