...
- @ModelAttribute 사용자가 요청시 전달하는 값을 오브젝트 형태로 매핑해주는 어노테이션.(다수)
...
####20210716(월)작업.
- 화페매수구하기:277550원 입금금액이 있다면,
- 5만원 짜리 지폐는 몇장인지,
- 1만원 짜리 지폐는 몇장인지,
- 5천원 짜리 지폐는 몇장인지,
- 1천원 짜리 지폐는 몇장인지,

-빅 O 사간복잡도 구하기:for문을 1개면, BigO=N,
-중목for이면,Big O=N^2
- for(i=i,i==3,i++){for(i=1,i==3,i++}{구현로직}
- 위 중복 for문은 시간복잡도가 N^2
- 프로그램의 성능을 측정하는 단위 빅O표기사용합니다. 
