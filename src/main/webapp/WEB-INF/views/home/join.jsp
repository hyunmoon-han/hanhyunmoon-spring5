...
$(document).ready(function() {
	$('.appForm').validate({
		rules:{
			password:"required",// password 인풋 태그의 이름,입력값 공백 체크
...
<div id="container">
	...
	<div class="bodytext_area box_inner">
		...
		<form method="POST" name="join_form" action="/join" class="appForm">
			<fieldset>
				<legend>회원가입폼</legend>
				<p class="info_pilsoo pilsoo_item">필수입력(회원가입 후 관리자가 승인해 줘야지만, 로그인이 가능합니다!)</p>
				<ul class="app_list">
					<li class="clear">
						<label for="user_id_lbl" class="tit_lbl pilsoo_item">사용자ID</label>
						<div class="app_content"><input type="text" name="user_id" class="w100p" id="user_id_lbl" placeholder="사용자 ID를 입력해주세요." required/></div>
					</li>					
					<li class="clear">
					...
					<li class="clear">
						<label for="password_chk_lbl" class="tit_lbl pilsoo_item">암호 확인</label>
					...
					<li class="clear">
						...
						<div class="app_content"><input type="email" name="email" class="w100p" id="email_lbl" placeholder="email을 입력해주세요." required/></div>
					</li>
					<li class="clear">
						<label for="point_lbl" class="tit_lbl pilsoo_item">point</label>
						<div class="app_content"><input value="0" readonly type="digits" name="point" class="w100p" id="point_lbl" placeholder="point를 입력해주세요." required/></div>
					</li>
					<li class="clear">
						<label for="gender_lbl" class="tit_lbl pilsoo_item">로그인여부</label>
						<div class="app_content ">							
							<input checked type="radio" value="0" name="enabled" class="css-radio" id="enabled_lbl" />
		...
		if($(this).val() !=""){
			$.ajax({
				...
				success:function(result){
					...
					}else{//중복아이디가 존재할떄 아래실행
						$("#btn_insert").attr("dlsabled",true);
