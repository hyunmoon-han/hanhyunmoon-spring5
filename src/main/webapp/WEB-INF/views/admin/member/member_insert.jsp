        ...
        <div class="card card-primary">
          ...
          <form name="form_write" action="/admin/member/member_insert" method="POST" enctype="multipart/form-data">
            <div class="card-body">
              ...
              <!-- 신규등록시 ID중복체크필수:버튼이벤트로처리 -->
                <label for="user_id">사용자ID
                <button id="btn_id_check" type ="button" class="btn btn-sm btn-secondary">중복체크</button>
                </label>
                <input value="" name="user_id" type="text" class="form-control" id="user_id" placeholder="회원 ID를 입력해 주세요" required>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
	            ...
	            <div class="form-group">
	                ...
	                <select name="enabled"id="enabled" class="form-control">
	                  <option value="1" select>허용</option>
	              ...
	              <div class="form-group">
	                ...
	                <select name="levels"  id="levels" class="form-control">
	                  <option value="ROLE_USER" select>사용자</option>
            ...
            <div class="card-footer text-right">
              ...
              <button type="button" class="btn btn-default"id="btn_list">목록</a></button>
            </div>
...
$(document).ready(function(){
	...
	$("#btn_id_check").click(function(){
		...
		//alert(user_id);
		$.ajax({
			type:"get",//입력,수정,삭제 아니면 get방식
			url:"/id_check?user_id="+user_id,//RestAPPI서버(스프링클래스로 제작)의 URL
			dataType:"text",//결과값(0,1지정해놈)을 받을때, 테이터형을 text,json,xml중 선택
			success:function(result){//(데이터타입])
				alert(result);
				if(result==0){//중복ID가 없다면 정상진행
					$("#btn_insert").attr("disabled",false);//등록버튼을 활성화 
					alert("사용가능한 ID입니다. ");
				}
				if(result==1){//중복ID가 있다면 진행중지
					$("btn_insert").attr("disabled",true);//등록버튼비활성화
					alert("올바르지않거나,중복ID가존재합니다.다시입력해주세요 ");
				}
			},
			error:function(){
				alert('RestAPI서버가 문제발생.담에써라');
			}
		});
	});
	var form_write =$("form[name='form_write']");//아이디값이없는 폼 $("form[name='form_write']")	
