...
public class ReplyController {
	...
	//댓글은 Read가필요없음,왜냐하면, ,Select로 가져온 값을 Ajax로처리하기 떄문에 쿼리를 날릴 필요가 없습니다.
	//그래서,바로 Update를 처리힙니다.-간단하게update시 read쿼리가 없고,Ajax처리함.
	...
	public ResponseEntity<String> reply_update(@RequestBody ReplyVO replyVO){
		//@RequestBody jsp에서 $.ajax를 이용해서 보내는 테이터값<->@ResponseBody응답값
		ResponseEntity<String> result=null;
		try {
		...
		return result;//Restful방식은 항상 반환값이(Body:String,Hashmap-json부분)존재합니다.
	}
	//댓글 등록@RequestBody는 jsp에서 Ajax메서드로 보내온 값을 받을때 사용하는 에노테이션
	...
	public ResponseEntity<String> reply_insert(@RequestBody ReplyVO replyVO){
		//ResponseEntity==ResponsBody 동급
		ResponseEntity<String> result=null;
		//개발자가 스프링에 예외를 throws하지 않고,직접try~catch하는 목적:Rest상태값(200,204,500,..등의 상황들)을 개발자가 보내줘야 하기 때문에
		try {
			replyService.insertReply(replyVO);
			result = new ResponseEntity<String>("success",HttpStatus.OK);//객체생성시 매개변수로 상태값+입력성공시 success라는 문자열도 보냄
