...
- UI디자인 과제물제출4교시
-
- 관리자단 게시판 생성관리 CRUD 처리 마무리.(3.스프링의 AOP기능구현).
...
- 2.게시판 생성관리 매퍼파일:
- 3.게시판 생성관리 DAO파일:
- 4.게시판 생성관리 Service파일
- 10,20년 ,지금까지 변하지 않는것은 변수값의 흐름은 변함이 없음.정방향개발시작
- 시작,VO->매퍼쿼리.xml->DAO클래스 생성->Service클래스생성->콘트롤러->jsp화면처리.
- 관리자단 게시물관리 CRUD 처리(4.파일업로드구현,5.트랜잭션구현).
